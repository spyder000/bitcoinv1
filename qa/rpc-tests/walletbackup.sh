...
B4ARGS="-datadir=$D4"
$BITCOINV1D $BITCOINV1DARGS $B4ARGS &
B4PID=$!
...
B3ARGS="-datadir=$D3"
$BITCOINV1D $BITCOINV1DARGS $B3ARGS &
B3PID=$!
...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} btc1"
...
function StartThree {
  $BITCOINV1D $BITCOINV1DARGS $B1ARGS &
  B1PID=$!
  $BITCOINV1D $BITCOINV1DARGS $B2ARGS &
  B2PID=$!
  $BITCOINV1D $BITCOINV1DARGS $B3ARGS &
  B3PID=$!
}
