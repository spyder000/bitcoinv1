...
{
    CBitcoinV1Address addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered bitcoinv1 address / amount:
            ...
            scriptPubKey.SetDestination(CBitcoinV1Address(rcp.address.toStdString()).Get());
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, transaction.getRecipients())
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal bitcoinv1:URI (bitcoinv1:123...?message=example)
...
{
    QString strAddress = QString::fromStdString(CBitcoinV1Address(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        mapCoins[CBitcoinV1Address(address).ToString().c_str()].push_back(out);
    }
}
...
{
    CTxDestination dest = CBitcoinV1Address(sAddress).Get();
