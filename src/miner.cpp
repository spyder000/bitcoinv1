...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BitcoinV1Miner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitcoinV1Miner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitcoinV1Miner(CWallet *pwallet)
{
    LogPrintf("BitcoinV1Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoinv1-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running BitcoinV1Miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("BitcoinV1Miner terminated\n");
        throw;
    }
}
...
void GenerateBitcoinV1s(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoinV1Miner, pwallet));
}
